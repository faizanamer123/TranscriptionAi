import os, shutil, asyncio, logging, multiprocessing, psutil, torch, uvicorn
...
def persistent_worker(task_queue, shared_state, job_to_worker, idx):
    ...
    except Exception as e: logger.error(f"Worker {os.getpid()} error: {e}")
...
manager, shared_state, task_queue, job_to_worker, worker_pool = None, None, None, None, []
        ...
        if not shared_state.get("is_paused", False):
            for j in jobs:
                if j["id"] in shared_state:
                    j.update(shared_state[j["id"]])
                    if j["status"] in ["completed", "failed", "cancelled"] and os.path.exists(j["path"]):
                        try: os.remove(j["path"])
                        except: pass
            active_ids.clear(); active_ids.update([j["id"] for j in jobs if j["status"] == "processing"])
...
async def lifespan(app):
    global manager, shared_state, task_queue, job_to_worker
    manager = multiprocessing.Manager(); shared_state, task_queue, job_to_worker = manager.dict(), manager.JoinableQueue(), manager.dict()
...
async def upload(files: list[UploadFile]):
    ...
    for f in files:
        ...
        jid = shared_state["job_id_counter"]; path = os.path.join("uploads", f"{jid}_{f.filename}")
        ...
        jobs.append(j); added.append(j); shared_state["job_id_counter"] += 1
...
@app.post("/pause")
async def pause(): shared_state["is_paused"] = True; return {"status": "paused"}

@app.post("/resume")
async def resume(): shared_state["is_paused"] = False; return {"status": "resumed"}

@app.post("/cancel/{jid}")
async def cancel(jid: int):
    j = next((j for j in jobs if j["id"] == jid), None)
    if not j: raise HTTPException(404)
    if j["status"] == "processing" and jid in job_to_worker:
        pid = job_to_worker.pop(jid)
        for i, p in enumerate(worker_pool):
            if p.pid == pid: p.kill(); worker_pool.pop(i); sync_pool(shared_state["allowed_workers"]); break
    j["status"] = "cancelled"; return {"status": "cancelled"}
